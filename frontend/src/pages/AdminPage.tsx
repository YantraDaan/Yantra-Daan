...
const AdminPage = () => {
  ...
  // Bulk operations removed - no longer needed
  ...
  const fetchTeamMembers = async () => {
    try {
      ...
      const response = await fetch(`${config.apiUrl}/api/admin/team-members?limit=50`, {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
  ...
  const deleteUser = async (userId, userName) => {
    const confirmed = window.confirm(`⚠️ WARNING: Are you sure you want to permanently delete user "${userName}"? This action cannot be undone!`);
    if (!confirmed) return;
                    ...
                    <>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {allDevices.map((device) => (
                          <Card key={device._id} className="group bg-white hover:shadow-xl transition-all duration-300 border-0 shadow-md hover:scale-[1.02] overflow-hidden">
                            <div className="relative">
                              {/* Status Badge */}
                              <div className="absolute top-3 right-3 z-10">
                                <Badge className={`${getStatusBadgeColor(device.status)} shadow-sm`}>
                                  {device.status?.charAt(0).toUpperCase() + device.status?.slice(1)}
                                </Badge>
                              </div>
                              
                              {/* Device Icon Header */}
                              <div className="bg-gradient-to-br from-green-500 via-teal-500 to-blue-500 p-6 text-white">
                                <div className="flex items-center space-x-3">
                            ...
                            <CardContent className="p-4 space-y-4">
                              ...
                              <div className="space-y-3">
                                <div className="flex items-center text-sm text-gray-600">
                                  ...
                                  <span className="truncate">{device.ownerInfo?.name || 'Unknown Owner'}</span>
                                  {device.ownerInfo?.isVerified && (
                                ...
                                <div className="flex items-center text-sm text-gray-600">
                                  ...
                                  <span className="truncate">{device.ownerInfo?.email || 'No Email'}</span>
                                </div>
                                ...
                                {device.ownerInfo?.profession && (
                                  <div className="flex items-center text-sm text-gray-600">
                                    ...
                                    <span className="truncate">{device.ownerInfo.profession}</span>
                                  </div>
                                )}
                              </div>
                              ...
                              <div className="grid grid-cols-2 gap-2">
                                ...
                                {/* Reject/Reset Button */}
                                {device.status !== 'rejected' && device.status !== 'suspended' ? (
                                  <Button 
                                    onClick={() => {
                              ...
                              <Button
                                onClick={async () => {
                                  const confirmed = window.confirm(`Are you sure you want to delete "${device.deviceName}"? This action cannot be undone.`);
                                  if (confirmed) {
                                    try {
                                      setIsActionLoading(true);
                                      const token = localStorage.getItem('authToken');
                                      
                                      const response = await fetch(`${config.apiUrl}/api/admin/devices/${device._id}`, {
                                        method: 'DELETE',
                                        headers: {
                                          'Authorization': `Bearer ${token}`,
                                        },
                                      ...
                                      if (response.ok) {
                                        toast({
                                          title: "Success",
                                          description: `"${device.deviceName}" has been deleted successfully`,
                                        });
                                        ...
                                        toast({
                                          title: "Error",
                                          description: errorData.error || "Failed to delete device",
                                          variant: "destructive",
                                        });
                                      }
                                    } catch (error) {
                    ...
                    <div className="flex space-x-3">
                      <Button
                        onClick={() => {
                          ...
                          const requestsData = [
                            ['Request ID', 'Requester', 'Device Type', 'Status', 'Priority', 'Date', 'Notes'],
                            ['REQ001', 'John Doe', 'Laptop', 'Pending', 'High', new Date().toLocaleDateString(), 'Student needs for studies'],
                            ['REQ002', 'Jane Smith', 'Tablet', 'Approved', 'Medium', new Date().toLocaleDateString(), 'Teaching purposes'],
                            ['REQ003', 'Bob Johnson', 'Mobile', 'Processing', 'Low', new Date().toLocaleDateString(), 'Basic communication']
                          ];
                          
                          const csv = requestsData.map(row => row.join(',')).join('\n');
                          const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
                          const url = URL.createObjectURL(blob);
                          const link = document.createElement('a');
                          link.href = url;
                          link.download = `device_requests_${new Date().toISOString().split('T')[0]}.csv`;
                          document.body.appendChild(link);
                          link.click();
                          document.body.removeChild(link);
                          URL.revokeObjectURL(url);
                          ...
                          toast({
                            title: "Success",
                            description: "Requests data exported successfully",
                          });
                        }}
                        className="bg-gradient-to-r from-green-600 to-yellow-500 text-white"
                      >
                  ...
                  <Card className="bg-white border border-gray-200 shadow-sm">
                    ...
                    <CardContent>
                        ...
                        <div className="border border-gray-200 rounded-lg p-4 hover:bg-gray-50 transition-colors">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-4">
                              ...
                              <div>
                                <h4 className="font-medium text-gray-900">Laptop Request - REQ001</h4>
                                <p className="text-sm text-gray-600">Requested by John Doe • Student</p>
                          ...
                          <div className="mt-3 pl-16">
                            <p className="text-sm text-gray-700">
                              "I am a computer science student and need a laptop for my studies and programming assignments. My current device is very old and slow."
                            </p>
                            <div className="mt-2 flex items-center space-x-4 text-xs text-gray-500">
                              <span>• Preferred: Laptop with 8GB+ RAM</span>
                        ...
                        <div className="border border-gray-200 rounded-lg p-4 hover:bg-gray-50 transition-colors">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-4">
                              ...
                              <div>
                                <h4 className="font-medium text-gray-900">Tablet Request - REQ002</h4>
                                <p className="text-sm text-gray-600">Requested by Jane Smith • Teacher</p>
                          ...
                          <div className="mt-3 pl-16">
                            <p className="text-sm text-gray-700">
                              "I teach in a rural school and need a tablet for interactive lessons and digital content delivery to my students."
                            </p>
                            <div className="mt-2 flex items-center space-x-4 text-xs text-gray-500">
                              <span>• Preferred: Android tablet 10"+ screen</span>
                        ...
                        <div className="border border-gray-200 rounded-lg p-4 hover:bg-gray-50 transition-colors">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-4">
                              ...
                              <div>
                                <h4 className="font-medium text-gray-900">Mobile Request - REQ003</h4>
                                <p className="text-sm text-gray-600">Requested by Bob Johnson • Unemployed</p>
                                <p className="text-xs text-gray-500">3 days ago</p>
                              </div>
                            </div>
                            <div className="flex items-center space-x-3">
                              <Badge className="bg-gray-100 text-gray-800">Low Priority</Badge>
                          ...
                          <div className="mt-3 pl-16">
                            <p className="text-sm text-gray-700">
                              "I recently lost my job and need a basic smartphone for job search and communication with potential employers."
                            </p>
                            <div className="mt-2 flex items-center space-x-4 text-xs text-gray-500">
                              <span>• Preferred: Basic Android phone</span>
                      ...
                      <div className="flex justify-center items-center space-x-4 mt-6">
                        ...
                        <span className="text-sm text-gray-600">
                          Page 1 of 5 (156 total requests)
                        </span>
                        <Button variant="outline">
                          Next
                        </Button>
                        ...
                        >
                          ...
                          <div className="relative h-48 overflow-hidden rounded-t-lg">
                            {member.avatar ? (
                              <>
                                <img 
                                  src={member.avatar.startsWith('http') ? member.avatar : `${config.apiUrl}/uploads/team-members/${member.avatar}`}
                                  alt={member.name}
                                  className="w-full h-full object-cover"
                                  onError={(e) => {
                                    console.log('Image failed to load:', member.avatar);
                                    const target = e.target as HTMLImageElement;
                                    target.style.display = 'none';
                                  }}
                                  onLoad={() => {
                                    console.log('Image loaded successfully:', member.avatar);
                                  }}
                                />
                                <div 
                                  className={`absolute inset-0 w-full h-full bg-gradient-to-br ${getRoleColor(member.role)} flex items-center justify-center`}
                                  style={{ display: 'none' }}
                              ...
                              <div className={`w-full h-full bg-gradient-to-br ${getRoleColor(member.role)} flex items-center justify-center`}>
                                <User className="w-16 h-16 text-white" />
                              </div>
                            )}
                  ...
                  <div className="flex flex-col md:flex-row items-center md:items-start space-y-4 md:space-y-0 md:space-x-6">
                    ...
                    <div className="flex-shrink-0 relative">
                      {selectedTeamMember.avatar ? (
                        <>
                          <img 
                            src={selectedTeamMember.avatar.startsWith('http') ? selectedTeamMember.avatar : `${config.apiUrl}/uploads/team-members/${selectedTeamMember.avatar}`}
                    ...
                    <div className="flex flex-col items-center space-y-4">
                      ...
                      <div className="flex-shrink-0 relative">
                        {editImagePreview ? (
                          ...
                          <img 
                            src={editingTeamMember.avatar.startsWith('http') ? editingTeamMember.avatar : `${config.apiUrl}/uploads/team-members/${editingTeamMember.avatar}`}
